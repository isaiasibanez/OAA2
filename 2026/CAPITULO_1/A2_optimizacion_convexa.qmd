...
<figure style="text-align: center;">
  ...
  <figcaption>**Figura 1.2.6:** Envolvente convexa y envolvente cónica de un mismo conjunto de puntos.</figcaption>
</figure>
</div>
...
<figure style="text-align: center;">
  ...
  <figcaption>**Figura 1.2.9:** $\Delta^2$ es un triángulo en $\RR^3$ de vértices $(1,0,0)$, $(0,1,0)$ y $(0,0,1)$.</figcaption>
</figure>
...
::: {.definicion}
**Definición 1.2.11** (Cono normal). Sean $\Omega\subset\RR^p$ un conjunto convexo y $\xx_0\in\Omega$. El *cono normal de $\Omega$ en $\xx$* se define como
$$
...
De manera análoga, una función es cóncava si y solo si su *hipógrafo*, definido como
$$
...
Pero $g(h)\to \varphi''(t_0)$ cuando $h\to 0$. Luego, $\varphi''(t_0)\ge 0$.
...
($\Leftarrow$) Supogamos que es cierta la desigualdad (1.2.4) del enunciado y probemos que $f$ es convexa. Sean $\xx_1,\xx_2\in\dom f$ y $\theta\in[0,1]$, definamos
$$
...
que es precisamente la definición de convexidad.
...
Una consecuencia importante del Teorema 1.2.19 es la siguiente: si para algún $\xx_0\in\dom f$ se verifica $\nabla f(\xx_0) =\bfzero$, entonces $f(\xx) \geq f(\xx_0)$ para todo $\xx \in \dom f$. De inmediato podemos afirmar que:
...
Como $f$ es convexa y dos veces diferenciable, podemos afirmar que también $\psi$ es convexa y dos veces diferenciable en $t$. Por lo tanto, el Lema 1.2.18 nos asegura que $\psi''(t)\geq 0$ para todo $t$. Derivando, obtenemos
$$
...
lo cual es la condición de primer orden (Teorema 1.2.19) aplicada a $\xx_1=\xx$ y $\xx_2=\xx+\dd$. Luego, $f$ es convexa.
...
es convexa, ya que para todo $\xx=(x,y)\in\RR^2$ se verifica
$$
\xx^\top A\xx=2x^2+2xy+3y^2=x^2+(x+y)^2+2y^2\geq 0,
$$
lo cual significa que $A\succeq 0$.
:::
...
Toda norma $\|\cdot\|$ en $\RR^p$ es una función convexa. En efecto, si $f(\xx)=\|\xx\|$, la desigualdad (1.2.3) de la definición de convexidad es inmediata de la desigualdad triangular:
$$
f(\theta\xx_1+(1-\theta)\xx_2)=\|\theta\xx_1+(1-\theta)\xx_2\|\leq\theta\|\xx_1\|+(1-\theta)\|\xx_2\|.
$$

Analicemos las condiciones de convexidad para el caso particular de la norma euclídea $f(\xx)=\|\xx\|_2=(\xx^\top\xx)^{1/2}$. Resultan
$$
...
$$
con $\dom f = \dom f_1 \cap \dom f_2$, también es convexa. Además, este resultado se puede extender a un conjunto $\{f_1,\ldots,f_m\}$ de funciones convexas.
...
<details>
<summary><span style="color:#0056b3;">ℹ️ ¿Por qué?</span></summary>
<hr>
...
es de optimización convexa. La función objetivo es una función convexa (ver Ejemplo 1.2.21) y la única restricción está definida a partir de $g(\xx)=-\bfone^\top\xx=-(x_1+\cdots+x_n)$, la cual es una función lineal (y, por lo tanto, convexa).
...
Es suficiente mostrar que $f(\zz)<f(\xx)$ para contradecir (1.2.8) y concluir la prueba. Para ello, basta usar el hecho de que $f$ es una función convexa:
$$
