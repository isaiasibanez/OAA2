...
Por último, del mismo modo que escribimos $A\xx=\bb$ para referirnos a un conjunto de igualdades de la forma $\bfa_i^\top\xx=b_i$, vamos a denotar $A\xx\preceq\bb$ al conjunto de desigualdades $\bfa_i^\top\xx\leq b_i$.
...
- El *interior relativo* de $C$ respecto a $\aff C$ es
    $$
...
$$
\aff C=\{\xx\in\RR^3 \mid x_3=0\}.
$$

Como consecuencia, a pesar de que el interior de $C$ es vacío ($\interior C=\emptyset$), el interior relativo es
$$
\relinterior C=(-1,1)\times(-1,1)\times\{0\}.
$$
...
Como su nombre lo indica, la envolvente convexa $\conv C$ es siempre un conjunto convexo (📝 Ejercicio 3). 

### <span style="font-size: 1.25em;">&#x2461;</span>

La envolvente convexa $\conv C$ es el conjunto convexo más pequeño que contiene a $C$: si $E$ es cualquier conjunto convexo que contiene a $C$, entonces $\conv C \subset E$. 
...
La envolvente cónica $\cone C$ es siempre un conjunto convexo (📝 Ejercicio 3). 

### <span style="font-size: 1.25em;">&#x2461;</span>

La envolvente cónica $\cone C$ es el cono convexo más pequeño que contiene a $C$: si $E$ es cualquier cono convexo que contiene a $C$, entonces $\cone C \subset E$. 
...
Sea $\xx_0\in\RR^2$ y $P\in\bfS_{++}^2$. El *elipsoide*
$$
...
El vector $\bfa$ es *normal* al hiperplano. Dado un punto $\xx_0$ del hiperplano, se verifica $\bfa^\top (\xx-\xx_0)=0$ para todo punto $\xx$ del hiperplano.

### <span style="font-size: 1.25em;">&#x2461;</span>

El escalar $b$ determina el desplazamiento del hiperplano respecto del origen. En efecto, la distancia del hiperplano al origen es $b/\|\bfa\|_2$ (probaremos esto más adelante en este capítulo).
...
<figure style="text-align: center;">
  ...
  <figcaption>**Figura 5**. Hiperplano en $\RR^2$ con vector normal $\bfa$.</figcaption>
</figure>
...
$$
\bfa_i^\top\xx\leq b_i,\qquad i=1,\ldots,r,
$$
$$
...
:::{.columns}
::: {.column width="70%"}
El cono normal a un punto $\xx_0\in\Omega$ es la recta con dirección $\bfa$. Esto es,
$$
...
:::{.columns}
::: {.column width="70%"}
- Si $\xx_0$ está en el interior de $\Omega$, $\bfa^\top\xx_0<b$, el cono normal es $N_{\Omega}(\xx_0)=\{\mathbf{0}\}$. 

- Si $\xx_0$ está en la frontera de $\Omega$, $\bfa^\top\xx_0=b$, resulta
$$
...
Sean $\bfa_1,\bfa_2\in\RR^p$ no nulos, y $b_1,b_2\in\RR$. Definimos la intersección de dos semiplanos como
$$
...
- Si $\xx_0$ está en la intersección de ambas fronteras, $\bfa_1^\top\xx_0=b_1$ y $\bfa_2^\top\xx_0=b_2$, entonces el cono normal es la envolvente cónica de las direcciones $\bfa_1$ y $\bfa_2$. Esto es
$$
...
::: {.definicion}
**Definición 8.** (Función convexa) Una función $f:\RR^p\rightarrow\RR$ es *convexa* si $\dom f$ es un conjunto convexo y si verifica
$$
...
$$
para todo $\xx_1, \xx_2 \in \dom f$, con $\xx_1 \neq \xx_2$.
...
$$
para todo $\xx_1, \xx_2 \in \dom f$.


### <span style="border: 1px solid #ddd; padding: 2px 6px; border-radius: 4px;">*Demostración*</span>

($\Rightarrow$) Supongamos que $f$ es convexa. Fijemos $\xx_1,\xx_2 \in \dom f$ y definamos
$$
...
($\Leftarrow$) Supogamos que es cierta la desigualdad (4) del enunciado y probemos que $f$ es convexa. Sean $\xx_1,\xx_2\in\dom f$ y $\theta\in[0,1]$, definamos
$$
...
Una consecuencia importante del Teorema 1 es la siguiente: si para algún $\xx_0\in\dom f$ se verifica $\nabla f(\xx_0) =\bfzero$, entonces $f(\xx) \geq f(\xx_0)$ para todo $\xx \in \dom f$. Es decir, podemos afirmar que:

:::{.myhighlight}
Si $f$ es una función convexa, cuyo $\dom f$ es un conjunto abierto, y para algún $\xx$ se verifica $\nabla f(\xx)=\bfzero$, entonces $\xx$ es un mínimo global de $f$.
:::
...
g(\xx):=f(A\xx+\bb)
$$
con $\dom g = \{\xx \mid A\xx + \bb \in\dom f\}$. Entonces, $g$ conserva la convexidad o concavidad de $f$.

- **Máximo puntual**. Si $f_1$ y $f_2$ son funciones convexas, entonces su *máximo puntual* $f$, definido por
$$
f(\xx) = \max\{f_1(\xx), f_2(\xx)\},
$$
con $\dom f = \dom f_1 \cap \dom f_2$, también es convexa. Además, este resultado se puede extender a: si $f_1,\dots,f_m$ son funciones convexas, entonces su máximo puntual, definido por $f(\xx)=\max\{f_1(\xx), \ldots, f_m(\xx)\}$, también lo es.
...
$$
\dom f = \{\xx \mid \xx \in \dom f_i (\forall i \in \mathcal{A}),\; \sup_{i \in \mathcal{A}} f_i(\xx) < \infty\}.
$$
...
Comparando con el problema en forma estándar general, podemos ver que las restricciones de igualdad, $h_i(\xx)=\bfa_j^\top \xx-b_i=0$, son funciones afines. Esto permite asegurar inmediatamente que:
...
- $\bigcap_{i=0}^r\dom f_i$.
- $r$ conjunto de subnivel $\{\xx \mid g_i(\xx)\leq 0\}$.
- $m$ hiperplanos $\{\xx \mid \bfa_i^\top\xx-b_i=0\}$.
