...
En esta sección nos enfocaremos en los conceptos fundamentales que sustentan la optimización convexa, una rama central en la teoría y práctica de la optimización. A partir de las ideas presentadas en el [C1-S1](A1_intro_optimizacion.html), donde destacamos el rol de la convexidad, ahora desarrollaremos con mayor profundidad las nociones teóricas que permiten formular y analizar este tipo de problemas.
...
La prueba queda como ejercicio (✏8).
...
$$
\nabla f(\xx):=\left(\frac{\partial f}{\partial x_1}(\xx),\ldots,\frac{\partial f}{\partial x_p}(\xx)\right).
$$
...
Para $r\leq p$, se define $f:\RR^p\to\RR$ como
$$
f(\xx)=\sum_{i=1}^rx_{[i]};
$$

esto es, a cada vector $\xx\in\RR^p$ le asigna la suma de sus $r$ componentes de mayor valor. Esta función puede ser reescrita como
$$
f(\xx)=\max\left\{x_{i_1}+\cdots+x_{i_r} \mid 1\leq i_1 \leq \ldots\leq i_r\leq n \right\};
$$

esto es, el máximo de todas las posibles sumas de $r$ diferentes componentes de $\xx$. Cada una de esas sumas es una función lineal (y, por lo tanto, convexa). En consecuencia, dado que el máximo puntual conserva la convexidad, concluimos que $f$ es convexa.
...
$$
g(\xx)=1-\bfone^\top\xx=1-(x_1+\cdots+x_n),
$$
la cual es una función afín y, por lo tanto, convexa.
...
$$
f(\xx)=\min\{f(\zz) \mid \zz\in\Omega,\; \|\zz-\xx\|_2\leq R\}.
\tag{1.2.8}
$$
...
Vamos a concluir esta sección haciendo un breve análisis sobre la existencia y unicidad de la solución. En primer lugar, un resultado importante es que, si la función objetivo $f$ es *estrictamente convexa*, entonces el problema admite, a lo sumo, una solución óptima (esto es, si hay punto óptimo, es *único*). En efecto, si hubiesen dos puntos óptimos distintos $\xx_1^\star$ y $\xx_2^\star$, el absurdo es inmediato: la convexidad estricta asegura que
$$
f(\theta\xx_1^\star+(1-\theta)\xx_2^\star)<\theta f(\xx_1^\star)+(1-\theta) f(\xx_2^\star)=p^\star,\qquad\forall\theta\in(0,1),
$$

lo cual contradice el hecho de que $p^*$ es el valor óptimo.
