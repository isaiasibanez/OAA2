...
Ahora, veamos que $x_1^\star\neq 0$ y $x_2^\star\neq 0$. Si $x_1^\star=0$ (análogamente para $x_2^\star$), de la primera ecuación en (1.4.8) se deduce $\lambda_2^\star=\lambda_1^\star-2$, tal que $\lambda_1^\star\geq 2$. La holgura complementaria resulta en $x_2^\star=1.5$, lo cual nos lleva una contradicción entre la segunda ecuación de (1.4.8), que resulta en $\lambda_3^\star=4$, y la holgura complementaria, que exige $\lambda_3^\star=0$.

En limpio, sabemos que debe ser $\lambda_1^\star\neq 0$, $x_1^\star\neq 0$, $x_2^\star\neq 0$. Además, por holgura complementaria, $\lambda_2^\star=0$ y $\lambda_3^\star=0$.

Luego, las condiciones KKT se reducen al siguiente sistema, que resulta de agregar a (1.4.10) la restricción activa:
$$
\begin{pmatrix}
1&0&1\\
0&4&1\\
...
Es importante notar que $\nabla \tilde{g}_i(\xx)=\nabla g_i(\xx^\star)$ y $\nabla \tilde{h}_i(\xx)=\nabla h_i(\xx^\star)$. En consecuencia, la aplicación del Teorema 1.4.7 a $\tilde{\Omega}$ resulta en el siguiente cono normal:
...
donde $I(\xx^\star):=\left\{i\in\{1,\cdots,r\} \mid g_i(\xx^\star)=0\right\}$ es el conjunto de restricciones activas. Por supuesto, el uso del conjunto índice $I$ puede evitarse utilizando holgura complementaria, tcomo hemos visto anteriormente. 
...
::: {.definicion}
**Definición 1.4.11** (Condiciones de Karush-Kuhn-Tucker (KKT)).
...
<span class="badge bg-primary">Ejemplo 1.4.12</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
Además, la condición de convexidad tampoco puede relajarse, como se ve en el Ejemplo 1.4.12, donde la función de restricción $g_1(x,y)=y-(1-x)^3$ no es convexa en un entorno del punto óptimo $(1,0)$.
...
::: {.teorema}
**Teorema 1.4.13** (Caracterización de optimalidad mediante KKT). 
...
$$
donde la primera igualdad es cierta gracias a la condición de holgura complementaria $\lambda_i g_i(\xx_0)=0$ para todo $i=1,\ldots,r$. Luego, $\xx_0$ es punto óptimo del problema, tal como queríamos probar.
<div style="margin-top:-1em;"></div>
<div style="text-align: right;">$\blacksquare$</div>
...
<span class="badge bg-primary">Ejemplo 1.4.14</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
::: {.definicion}
**Definición 1.4.16** (Función dual). 
Sea $\calL$ el Lagrangiano de la Definición 1.4.15. Se denomina *función dual de Lagrange* a $\calG:\RR^r\times\RR^m\to\RR$ definida por
$$
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 1.4.17</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
::: {.callout .question}
<span style="font-size: 1.3em;">✍️</span><br>
Proponga casos para el Ejemplo 1.4.17, con $\xx\in\RR^2$ y $m=2$. En cada uno, de ser posible, obtenga y compare el valor máximo de la función dual con la solución del problema primal.
...
::: {.definicion}
**Definición 1.4.18** (Problema dual). 
...
::: {.teorema}
**Teorema 1.4.19** (Teorema de Slater). 
Sea un problema de optimización convexa de la forma
$$ 
\begin{array}{ll} 
\text{minimizar } & f(\xx)\\ 
...
<span class="badge bg-primary">Ejemplo 1.4.20</span> 

Consideremos el problema del Ejemplo 1.4.19, con $m=1$:
$$
\begin{array}{ll}
\text{minimizar } & \xx^\top\xx\\
...
Dado que se cumplen las condiciones del Teorema 1.4.13 (problema de optimización convexa + condición de Slater), podemos concluir que hay dualidad fuerte. Luego, el valor óptimo del problema primal es
$$
...
A partir de (15) y (16) podemos concluir que, bajo dualidad fuerte, la terna $(\xx^\star,\bflambda^\star,\bfnu^\star)$ verifica dos condiciones KKT de la Definición 1.4.11: holgura complementaria y estacionariedad.
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 1.4.22</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
