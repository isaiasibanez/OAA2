...
Si no hay restricciones de desigualdad y las restricciones de igualdad son afines, la cualificación de LICQ se satisface si y solo si el conjunto de vectores normales es linealmente independiente. Esta situación es habitual ya que, en caso de que los vectores normales sean linealmente dependientes, o bien alguna de las restricciones es redundante o bien el sistema de igualdades es incompatible. 
	
En consecuencia, en problemas de la forma
$$
\begin{array}{ll} 
	\text{minimizar } & f(\xx)\\ 
	\text{sujeto a } & A\xx=\bb,
...
la existencia de un punto factible es suficiente para garantizar que las condiciones KKT se satisfacen en todo punto óptimo.
...
con $\dom\calL:=\mathcal{D}\times\RR^r\times\RR^m$. Las componentes de los vectores $\bflambda$ y $\bfnu$ se denominan *multiplicadores de Lagrange*.
:::
...
::: {.definicion}
**Definición 1.4.17** (Función dual). 
...
::: {.callout .question}
<span style="font-size: 1.3em;">✍️</span><br>
Proponga casos para el Ejemplo 1.4.18, con $\xx\in\RR^2$ y $m=2$. En cada uno, de ser posible, obtenga y compare el valor máximo de la función dual con la solución del problema primal.
...
::: {.definicion}
**Definición 1.4.19** (Problema dual). 
...
Recordemos, del [C1-S2](A2_optimizacion_convexa.html), que $\relinterior \mathcal{D}$ es el interior relativo de $\mathcal{D}$ definido por
$$
\relinterior \mathcal{D}:=\left\{\xx\in \mathcal{D}\,\left|\exists r>0: B(\xx,r)\cap \aff\mathcal{D}\subset \mathcal{D}\right.\right\},
$$


La aparición de $\relinterior\mathcal{D}$ sirve para sortear posibles casos degenerados, en los que $\mathcal{D}$ esté contenido en un subespacio afín de $\RR^p$ y, por lo tanto, tenga interior vacío. Sin embargo, lo común es que $\relinterior\mathcal{D}=\interior\mathcal{D}$, tal como sucede para problemas donde $\dom f=\RR^p$ y $\dom g_i=\RR^p$ . Además, si solo hay restricciones de igualdad, la condición de Slater se reduce simplemente a verificar que $\Omega$ sea no vacío.
...
$$
\nu^\star=\frac{2b}{\|\bfa\|_2^2},\qquad d^\star=\calG(\nu^\star)=-\frac{3b^2}{\|\bfa\|_2^2}.
$$

Dado que se cumplen las condiciones del Teorema 1.4.20 (problema de optimización convexa + condición de Slater), podemos concluir que hay dualidad fuerte. Luego, el valor óptimo del problema primal es
$$
p^\star=-\frac{3b^2}{\|\bfa\|_2^2}.
$$
...
::: {.callout .question}
<span style="font-size: 1.3em;">✍️</span><br>
Proponga casos para el Ejemplo 1.4.20, con $\xx\in\RR^2$, y deduzca geométricamente el punto óptimo $\xx^\star$.
...
Hay que tener cuidado al interpretar la expresión (1.4.13): si bien garantiza que $f(\xx^\star)$ es el valor ínfimo del Lagrangiano, no nos dice nada acerca de cuál es el valor de $\xx$ donde dicho ínfimo se alcanza. No obstante, la naturaleza de los multiplicadores de Lagrange y las restricciones funcionales nos permiten deducir, a partir de (1.4.13), lo siguiente:
$$
\begin{align*}
...
A partir de (1.4.13) y (1.4.14) podemos concluir que, bajo dualidad fuerte, la terna $(\xx^\star,\bflambda^\star,\bfnu^\star)$ verifica dos condiciones KKT de la Definición 1.4.12: holgura complementaria y estacionariedad.
...
Para que la igualdad en (1.4.14) sea cierta, debe ser
$$
