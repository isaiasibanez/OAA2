...
las condiciones de optimalidad definen los requisitos que deben cumplir los puntos óptimos. En lo que sigue, <mark>asumiremos siempre que trabajamos con funciones diferenciables</mark>.
...
De cursos anteriores recordemos que, cuando se pretende optimizar una función $f$ en su dominio, una condición necesaria (siempre que $\dom f$ sea un  conjunto abierto) para que un punto sea óptimo es que verifique 
$$
\nabla f(\xx)=\mathbf{0}.
\tag{1.4.1}
$$
...
$$
f(\xx + t\dd) \geq f(\xx) \qquad \text{para todo } 0\leq t<\varepsilon,
$$
...
Para el caso de optimización sin restricciones, podemos responder a la segunda pregunta con un resultado muy importante en optimización convexa, relacionado con el Teorema 1.2.3 del [C1-S2](A2_optimizacion_convexa.html#problemas-de-optimización-convexa).
...
Para generalizar la condición necesaria $\nabla f(\xx)=\bfzero$ al caso de optimización con restricciones, es importante respetar el sentido de una dirección $\dd\in\RR^p$, razón por la cual vamos a restringir el movimiento $\xx+t\dd$ a valores no negativos del parámetro; esto es, $t\geq 0$. De esta manera, la posibilidad de aproximarnos a un punto factible $\xx$ sin salirnos del conjunto factible $\Omega$ va a depender exclusivamente de la dirección $\dd$ considerada. Esto conduce a una generalización natural de la condición de optimalidad provista por la prueba anterior, a saber:
...
para cualquier $t>0$ lo suficientemente pequeño. A partir de esto, nos preguntamos:
...
Bajo esta suposición, tenemos el siguiente resultado, cuya demostración se obtiene directamente de ajustar la prueba vista para (1.4.1).
...
<figure style="text-align: center;">
  ...
  <figcaption> **Figura 1.4.1:** Interpretación geométrica de la condición necesaria (1.4.3) del Teorema 1.4.2.   </figcaption>
</figure>

<div style="margin-top:2em;"></div>


Como se puede apreciar en la Figura 1.4.1, para un punto óptimo $\xx^\star$ el vector gradiente forma un ángulo agudo (o recto) con todas las direcciones de la forma $\yy-\xx^\star$. Esto se deduce del hecho que
$$
...
Ahora bien, nosotros estamos interesados en hallar $\xx^\star$. En este sentido, la condición (1.4.3) nos sirve para identificar aquellos puntos $\xx\in\Omega$ candidatos a ser óptimos (siguiendo la terminología habitual, los llamaremos <mark>*puntos críticos*</mark>). En consecuencia, si consideramos el vector opuesto al gradiente, los puntos críticos serán aquellos que verifiquen 
$$
\langle -\nabla f(\xx),\yy-\xx\rangle_2\leq 0\qquad \forall\yy\in\Omega.
\tag{1.4.4}
$$

Esto vincula nuestro análisis con la definición de cono normal (Definición 7 de [C1-S2](A2_optimizacion_convexa.html#conos-normales)): la expresión (1.4.4) es equivalente a  escribir $-\nabla f(\xx)\in N_{\Omega}(\xx)$. Así, el Teorema 1.4.2 puede ser interpretado en términos de conos normales como sigue:
...
Pese a verificar la condición necesaria (1.4.3), el punto $(0,0)$ no es óptimo. De hecho, $f(0,0)=0$, mientras que el valor óptimo es $-\frac{1}{4}$ y ocurre en los vértices $(\pm\frac{1}{2},0)$.
...
Para el notable caso de las <mark>funciones convexas</mark> ([Definición 8 de C1-S2](A2_optimizacion_convexa.html#funciones-convexas)), la condición establecida en el Teorema 1.4.2 sí es suficiente.
...
Por condición de primer orden para convexidad (Teorema 1.2.1 de [C1-S2](A2_optimizacion_convexa.html#condiciones-de-convexidad)), resulta
$$
...
Claramente $\nabla^2 f(\xx)\succeq 0$, lo cual significa que $f$ es convexa. Así, por Teorema 1.4.4, la condición (1.5.5) es suficiente y necesaria para que $\xx$ sea un punto óptimo. Esto nos permite hacer el siguiente análisis:
...
::: {.teorema}
**Teorema 1.4.7** (Cono normal a un punto de un poliedro).
...
Sea $\Omega$ el poliedro definido por (1.4.6), y sea $f:\RR^p\to\RR$ una función diferenciable. Si $\xx^\star\in \Omega$ es un minimizador de $f$ sobre $\Omega$, entonces existen $\bflambda\in(\RR_0^+)^r$ y $\bfnu\in\RR^m$ tales que $\lambda_i(\bfa_i^\top\xx-b_i)=0$ para todo $i=1,\ldots,r$ y
$$
