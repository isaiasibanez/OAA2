...
<div class="d-flex align-items-center justify-content-between">
  ...
  <a href="../material/apunte_c1s4.pdf" target="_blank" class="btn btn-outline-primary">
    <i class="fa-solid fa-file-lines me-2"></i>Versión PDF
  </a>

  <a href="../material/tp_c1s4.pdf" target="_blank" class="btn btn-outline-success">
...
## I <span style="color:#CCCCCC;">│</span> Condiciones de optimalidad de primer orden
...
$$
f(\xx + t\dd) \geq f(\xx) \qquad \forall t\in\RR: |t|<\varepsilon,
$$
...
$$
f(\xx^\star + t \dd) \geq f(\xx^\star) \qquad\forall t\in\RR:\xx^\star+t\dd\in\dom f.
$$
...
¿Cuál es la generalización de la condición necesaria $\nabla f(\xx) = \bfzero$ cuando enfrentamos un problema de optimización con restricciones?
...
::: {.teorema}
**Teorema 1.4.1** (Condición suficiente y necesaria de optimalidad de primer orden sin restricciones para funciones convexas). 
Sea $f:\RR^p\to\RR$ una función diferenciable y  convexa. Entonces $\xx^\star\in \dom f$ es un minimizador de $f$ si y solo si
$$
...
Para generalizar la condición necesaria $\nabla f(\xx)=\bfzero$ al caso de optimización con restricciones, es fundamental tener en cuenta que, en un punto dado, podemos estar limitados en la elección de las direcciones de movimiento $\dd\in\RR^p$. Para que la posibilidad de aproximarnos a un punto factible $\xx$ sin salirnos del conjunto factible $\Omega$ dependa de la dirección $\dd$ considerada, consideraremos únicamente desplazamientos de la forma $\xx+t\dd$, con $t\geq 0$. Es fácil ver que esta limitación modifica el análisis realizado en la prueba anterior, conduciendo a la siguiente generalización natural:
$$
\langle\nabla f(\xx^\star),\dd\rangle_2\geq 0\qquad \forall\dd\in\RR^p: \xx^\star+t\dd\in\Omega,
\tag{1.4.2}
$$
...
¿Cuáles son las direcciones $\dd$ que nos permiten permanecer en $\Omega$ desde un punto $\xx$?
...
::: {.teorema}
**Teorema 1.4.2** (Condición necesaria de optimalidad de primer orden para un conjunto factible convexo). 
...
<figure style="text-align: center;">
  <img src="figuras/angulo_entre_gradiente_xy.svg" alt="…" width="400px">
  <figcaption> **Figura 1.4.1:** Interpretación geométrica de la condición necesaria (1.4.3) cuando $\nabla f(\xx^\star)\neq 0$. </figcaption>
</figure>

<div style="margin-top:2em;"></div>


En un punto óptimo $\xx^\star$, el vector gradiente o bien es nulo o bien  forma un ángulo nulo, agudo o recto con todas las direcciones de la forma $\yy-\xx^\star$ (ver Figura 1.4.1). En efecto, si $\nabla f(\xx^\star)\neq\bfzero$, se verifica
$$
\langle \nabla f(\xx^\star), \yy - \xx^\star\rangle_2=\|\nabla f(\xx^\star)\|_2\|\yy-\xx^\star\|_2\cos \alpha\geq 0\iff \cos\alpha\geq 0.
$$


Ahora bien, nosotros estamos interesados en hallar $\xx^\star$. En este sentido, la condición (1.4.3) nos sirve para identificar aquellos puntos $\xx\in\Omega$ candidatos a ser óptimos (siguiendo la terminología habitual, los llamaremos *puntos críticos*). En consecuencia, si consideramos el vector opuesto al gradiente, los puntos críticos serán aquellos que verifiquen 
$$
\langle -\nabla f(\xx),\yy-\xx\rangle_2\leq 0\qquad \forall\yy\in\Omega.
\tag{1.4.4}
$$

Esto vincula nuestro análisis con la Definición 1.2.11 de cono normal: la expresión (1.4.4) es equivalente a  escribir $-\nabla f(\xx)\in N_{\Omega}(\xx)$. En consecuencia, podemos afirmar que:


:::{.myhighlight}
Bajo las condiciones del Teorema 1.4.2, si $\xx^\star$ es un minimizador de $f$ sobre $\Omega$, entonces
$$
...
<figure style="text-align: center;">
  <img src="figuras/condicion_optimalidad_gradiente_vs_cono_normal.svg" alt="…" width="450px">
  <figcaption>**Figura 1.4.2:** Idea geométrica de la condición del gradiente para tres puntos críticos $\xx_1,\xx_2,\xx_3\in\Omega$ y un punto no crítico $\xx_4\in\Omega$. En el caso de $\xx_3$, se tiene $\nabla f(\xx_3)=\bfzero$.</figcaption>
</figure>

<div style="margin-top:2em;"></div>


En la Figura 1.4.2 se presenta la geometría derivada de la condición necesaria establecida, para el caso en que $\Omega$ es un poliedro. Las posiciones de los puntos críticos corresponden a los ejemplos de conos normales vistos en el [C1-S2](A2_optimizacion_convexa.html#conos-normales). En particular, para puntos interiores de $\Omega$, la condición se reduce a la ecuación $\nabla f(\xx)=\bfzero$, típica de los problemas sin restricciones.

El comportamiento de $-\nabla f(\xx)$ en términos de conos normales constituye, sin dudas, la idea principal de nuestro análisis, que procuraremos extender a formas más generales de $\Omega$. Observar que la condición provista por el Teorema 1.4.2 es necesaria pero no suficiente. Podemos pensar rápidamente en ejemplos de mínimos locales en el interior de $\Omega$, tales como el Ejemplo 1.1.3. A continuación, veremos un ejemplo en el cual un punto crítico ocurre en la frontera de $\Omega$, pero no es cierto que sea un punto óptimo.


::: {.callout-example}

<span class="badge bg-primary">Ejemplo 1.4.3</span> 
...
Esto es cierto, de acuerdo a la definición de $\Omega$. Sin embargo, el punto $(0,0)$ no es óptimo: $f(0,0)=0$, mientras que el valor óptimo es $-\frac{1}{4}$ y ocurre en los vértices $(\pm\frac{1}{2},0)$.

<figure style="text-align: center;">
  <img src="figuras/ej_grad_cono_fail.svg" alt="…" width="300px">
</figure>
...
::: {.teorema}
**Teorema 1.4.4** (Condición suficiente y necesaria de optimalidad de primer orden para un conjunto factible convexo y una función  objetivo convexa). 
...
En virtud del Teorema 1.4.2, debemos probar únicamente la suficiencia. Sea $\xx\in\Omega$ tal que $-\nabla f(\xx)\in N_{\Omega}(\xx)$, entonces
$$
...
$$
para todo $\yy\in\Omega$, lo cual significa que $\xx$ es un minimizador de $f$ en $\Omega$. <div style="margin-top:-1em;"></div>
<div style="text-align: right;">$\blacksquare$</div>
...
<span class="badge bg-primary">Ejemplo 1.4.5</span>
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
con
$$
A=\begin{pmatrix}1&0\\0&2\end{pmatrix}\qquad\mbox{y}\qquad\bb=\begin{pmatrix}1\\1\end{pmatrix}. 
$$
...
Claramente $\nabla^2 f(\xx)\succeq 0$, lo cual significa que $f$ es convexa. En consecuencia, estamos en condiciones de aplicar el Teorema 1.4.4. Para ello, hagamos el siguiente análisis:
...
- En el lado $\{(t,1.5-t)\mid 0<t<1.5\}$,  tenemos que $-\nabla f(\xx)=(1-t,4t-4)$ y la dirección normal es $(1,1)$. Esto significa que (1.5.5) es cierto si ambas componentes de $-\nabla f(\xx)$ son iguales, lo cual resulta en $t=1$. De esta manera, el punto $(1,0.5)$, con $\nabla f(1,0.5)=\bfzero$, es un punto óptimo del problema.

- Faltan los vértices del triángulo. En la siguiente figura se muestra el comportamiento de $-\nabla f(\xx)$ en cada uno de ellos. Dado que no cumplen la condición suficiente y necesaria (1.5.5), ningun vértice es un punto óptimo.

<figure style="text-align: center;">
  <img src="figuras/ej_vertices_no_optimos.svg" alt="…" width="400px">
</figure>
...
## II <span style="color:#CCCCCC;">│</span> Condiciones de Karush-Kuhn-Tucker (KKT)
...
::: {.definicion}
**Definición 1.4.6** (Restricción activa). Una restricción de desigualdad $g(\xx)\leq 0$ se dice que está *activa* en un punto $\xx_0$ si allí se cumple con igualdad; es decir, si
$$
g(\xx_0)= 0.
$$
...
En los ejemplos de conos normales vistos en [C1-S2](A2_optimizacion_convexa.html#conos-normales), analizamos su definición según la posición del punto de interés: si $\xx$ es interior a $\Omega$, resulta simplemente $N_{\Omega}(\xx)=\{\bfzero\}$, mientras que si $\xx$ pertenece a la frontera de $\Omega$, el cono normal queda determinado por las direcciones que definen los semiespacios e hiperplanos. No obstante, en dichos ejemplos hemos considerado casos particulares de $\Omega$:  un hiperplano, un semiespacio y la intersección de dos semiespacios. Necesitamos extender ese análisis a cualquier poliedro $\Omega$ y caracterizar el cono normal en cualquier punto de $\Omega$. El siguiente resultado nos proporciona dicha caracterización.

<div style="margin-top:2em;"></div>


::: {.teorema}
**Teorema 1.4.7** (Cono normal en un punto de un poliedro).
Sea $\Omega$ el poliedro definido por (1.4.6). Para $\xx\in\Omega$, sea $I(\xx)$ el conjunto de índices de las restricciones activas; esto es,
$$
I(\xx):=\left\{i\in\{1,\cdots,r\} \mid \bfa_i^\top\xx=b_i\right\}.
$$
Entonces, el cono normal en $\xx$ está dado por

$$
N_{\Omega}(\xx)=\left\{\left.\sum_{i\in I(\xx)}\lambda_i\bfa_i+\sum_{j=1}^m\nu_j\cc_j\,\right| \lambda_i\in\RR_0^+,\;\nu_j\in\RR\right\}.
$$
:::
...
La suma en la expresión del cono normal puede ser reescrita sin restringir $i\in I(\xx)$, imponiendo $\lambda_i=0$ para todo $i\notin I(\xx)$. Esta imposición queda ímplicita de forma inmediata si se escribe

$$
\sum_{i=1}^r\lambda_i\left(\bfa_i^\top\xx-b_i\right)=0\qquad\lambda_i\in\RR_0^+.
\tag{1.4.7}
$$


En forma vectorial, (1.4.7) puede escribirse como $\bflambda^\top\left(A\xx-\bb\right)=0$, donde $\bflambda=(\lambda_1,\cdots,\lambda_m)^\top$. Con esta notación, podemos escribir el cono normal del Teorema 1.4.7 así:
$$
...
En primer lugar, fijemos $\lambda_i\in\RR_0^+$, $\nu_j\in\RR$, y probemos que la dirección
$$
\dd=\sum_{i\in I(\xx)}\lambda_i\bfa_i+\sum_{j=1}^m\nu_j\cc_j	
$$

pertenece a $N_{\Omega}(\xx)$: para $\yy\in\Omega$ arbitrario, resulta
$$
\begin{align*}
	\langle \dd,\yy-\xx\rangle_2 &= \big\langle \sum_{i\in I(\xx)}\lambda_i\bfa_i+\sum_{j=1}^m\nu_j\cc_j,\yy-\xx \big\rangle_2 \\
	%&=\sum_{i\in I(\xx)}\lambda_i\langle\bfa_i,\yy-\xx\rangle_2+\sum_{j=1}^m\nu_j\langle \cc_j,\yy-\xx\rangle_2 \\
	&= \sum_{i\in I(\xx)}\lambda_i\left(\langle\bfa_i,\yy\rangle_2-\langle\bfa_i,\xx\rangle_2\right)+\sum_{j=1}^m\nu_j\left(\langle \cc_j,\yy\rangle_2-\langle\cc_j,\xx\rangle_2\right) \\
	&\leq 0,
\end{align*}
$$

donde en el último paso se usó el hecho que $\langle\bfa_i,\yy\rangle_2\leq b_i$, $\langle\bfa_i,\xx\rangle_2=b_i$, $\langle\cc_j,\yy\rangle_2=\langle\cc_j,\xx\rangle_2=d_j$.
	
En segundo lugar, probamos que toda dirección $\dd\in N_{\Omega}(\xx)$ pertenece al conjunto
$$
C:=\left\{\left.\sum_{i\in I(\xx)}\lambda_i\bfa_i+\sum_{j=1}^m\nu_j\cc_j\,\right| \lambda_i\in\RR_0^+,\;\nu_j\in\RR\right\}
$$

Por reducción al absurdo, supongamos $\dd\notin C$. Dado que $C$ es un cono convexo cerrado no vacío (esto es fácil de verificar y queda como ejercicio), podemos separar $C$ y $\dd$ mediante un hiperplano; esto es, existe $\uu\in\RR^n$ tal que
$$
\langle \uu,\dd\rangle_2<0\qquad\text{y}\qquad \langle\uu,\mathbf{p}\rangle_2\geq 0\;\;\forall\mathbf{p}\in C.
$$
	
En particular, tenemos $\langle\uu,\bfa_i\rangle\geq 0$ para las restricciones activas y $\langle\uu,\cc_j\rangle_2=0$; esto último por el hecho de que $\pm\cc_j\in C$. Como consecuencia, podemos afirmar que para un $\delta>0$ lo suficientemente pequeño, el punto $\yy:=\xx-\delta\uu$ pertenece a $\Omega$. En efecto, se verifica
$$
\langle \bfa_i,\xx-\delta\uu\rangle_2=\langle\bfa_i,\xx\rangle_2-\delta\langle\bfa_i,\uu\rangle_2\leq b_i\qquad\forall i\in I(\xx),
$$
$$
\langle\cc_j,\xx-\delta\uu\rangle_2=\langle\cc_j,\xx)=d_j\qquad\forall j=1,\ldots,m,
$$

mientras que para las restricciones no activas se usa el hecho de que $\delta$ es suficientemente pequeño. 
	
Finalmente, esto nos conduce a contradecir $\dd\in N_{\Omega}(\xx)$, ya que
$$
\langle \dd,\xx-(\xx-\delta\uu)\rangle_2=-\delta\langle\dd,\uu\rangle_2>0.
$$
	
Luego, debe ser $\dd\in C$, lo cual concluye la prueba.
<div style="margin-top:-1em;"></div>
<div style="text-align: right;">$\blacksquare$</div>
...
Verifique que el resultado del Teorema 1.4.7 contempla los ejemplos de conos normales vistos en [C1-S2](A2_optimizacion_convexa.html#conos-normales).

:::

<div style="margin-top:2em;"></div>

La condición de optimalidad $-\nabla f(\xx^\star)\in N_{\Omega}(\xx^\star)$ del Teorema 1.4.4 se traduce como
$$
-\nabla f(\xx^\star)=\sum_{i\in I(\xx)}\lambda_i^\star\bfa_i+\sum_{j=1}^m\nu_j^\star\cc_j,\qquad\lambda_i^\star\in\RR^+,\;\nu_j^\star\in\RR.
$$

O, equivalentemente,
$$
-\nabla f(\xx^\star)=\sum_{i\in I(\xx)}\lambda_i^\star \nabla g_i(\xx)+\sum_{j=1}^m\nu_j^\star\nabla h_j(\xx),\qquad\lambda_i^\star\in\RR^+,\;\nu_j^\star\in\RR.
$$
donde $g_i(\xx)=\bfa_i^\top\xx-b_i$ y $h_j(\xx)=\cc_j^\top\xx-d_i$ son las funciones de restricción que definen $\Omega$. Los coeficientes $\lambda_i$ y $\nu_j$ son *multiplicadores* asociados a las restricciones.

Ahora bien, dado que $\Omega$ es un poliedro (y, por lo tanto, un conjunto convexo), los Teoremas 1.4.2 y 1.4.4 pueden ser aplicados en este contexto. Obtenemos así el siguiente resultado.


::: {.teorema}
**Corolario 1.4.8.** (Condición necesaria de optimalidad de primer orden para un conjunto factible poliedral) 
Sea $\Omega$ el poliedro definido por (1.4.6), y sea $f:\RR^p\to\RR$ una función diferenciable. Si $\xx^\star\in \Omega$ es un minimizador de $f$ sobre $\Omega$, entonces existen $\bflambda^\star\in(\RR_0^+)^r$ y $\bfnu^\star\in\RR^m$ tales que $\lambda_i^\star(\bfa_i^\top\xx^\star-b_i)=0$ para todo $i=1,\ldots,r$ y
$$
-\nabla f(\xx^\star)=\sum_{i=1}^r\lambda_i\bfa_i+\sum_{j=1}^m \nu_j\cc_j.
\tag{1.4.8}
$$  
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 1.4.9</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
  Verificación de condición necesaria de optimalidad
</span>
...
Las restricciones determinan $\bfa_1=(2,1)$, $\bfa_2=(-2,1)$ y $\bfa_3=(0,-1)$. Entonces debemos hallar $\lambda_1, \lambda_2, \lambda_3\in\RR_0^+$ tales que
$$
...
$$
-\nabla f(\tfrac{1}{2},0)=\frac{1}{2}(2,1)+\frac{3}{2}(0,-1).
$$
...
Las condiciones del Corolario 1.4.8, conocidas como *condiciones de Karush-Kuhn-Tucker* (KKT), se formulan a partir de diferentes componentes:

- **Factibilidad primal**: El punto $\xx$ debe ser un punto factible.

- **Factibilidad dual**: Los multiplicadores asociados a las restricciones de desigualdad deben ser no negativos.
$$
\lambda_i\geq 0 \qquad \forall i=1,\cdots,r.
$$

- **Holgura complementaria**: Los multiplicadores asociados a las restricciones de desigualdad solo pueden ser positivos si la restricción está activa.
$$
\lambda_i\left(\bfa_i^\top\xx-b_i\right)=0 \qquad \forall i=1,\cdots,r.
$$

- **Estacionariedad**: El opuesto del vector gradiente debe ser una combinación lineal de los gradientes de las restricciones.
      $$
      -\nabla f(\xx)=\sum_{i=1}^{r}\lambda_i\bfa_i+\sum_{j=1}^m\nu_j\cc_j,
      $$
...
Por supuesto, el Corolario 1.4.8 representa un criterio para obtener puntos críticos si $\Omega$ es un poliedro. Más aún, si $f$ es convexa, dicho criterio caracteriza los puntos óptimos. En resumen, hasta aquí tenemos que:

:::{.myhighlight}

Si $\Omega$ es un poliedro, entonces las condiciones KKT son necesarias para que un punto sea óptimo. Si, además, $f$ es convexa, las condiciones KKT son también suficientes.
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 1.4.10</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
  Minimización cuadrática convexa con restricciones de igualdad
</span>

Sean $P\in\bfS_+^p$, $\mathbf{q}\in\RR^p$, $c\in\RR$, $A\in\RR^{m\times p}$ y $\bb\in\RR^p$. Consideremos el problema
...
Dado que la función objetivo es convexa, el Corolario 1.4.8 nos asegura que las condiciones KKT son suficientes y necesarias. Además, teniendo en cuenta que solo hay restricciones de igualdad, a saber, $h_j(\xx)=\bfa_j^\top\xx-b_j$, únicamente debemos analizar las condiciones de factibilidad primal y estacionariedad. La condición de estacionariedad es:
...
::: {.callout .question}
<span style="font-size: 1.3em;">✍️</span><br>
Proponga casos para el Ejemplo 1.4.10, con $\xx\in\RR^2$ y $m=2$.
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 1.4.11</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
Dado que $P\succeq 0$, la función objetivo es convexa. Su expresión es
$$
	f(x_1,x_2)=\frac{1}{2}x_1^2+2x_2^2-2x_1-2x_2.
$$
  
Además, el conjunto factible $\Omega$ es un poliedro. Por lo tanto, por Corolario 1.4.8, las condiciones KKT son suficientes y necesarias para la optimalidad. Las definimos a continuación:
...
Ahora, veamos que $x_1^\star\neq 0$ y $x_2^\star\neq 0$. Si $x_1^\star=0$ (análogamente para $x_2^\star$), de la primera ecuación en (\ref{eq:sist_ecua_ej_11}) se deduce $\lambda_2^\star=\lambda_1^\star-2$, tal que $\lambda_1^\star\geq 2$. Además, (\ref{eq:holg_compl_ej_11}) resulta en $x_2^\star=1.5$. Así, de la segunda ecuación de (\ref{eq:sist_ecua_ej_11}) se deduce
$$
\lambda_3^\star=4+\lambda_1^\star\geq 6, 
$$

lo cual contradice la holgura complementaria $\lambda_3^\star x_2^\star=0$ que en este caso exige sea $\lambda_3^\star=0$.

En limpio, sabemos que debe ser $\lambda_1^\star\neq 0$, $x_1^\star\neq 0$, $x_2^\star\neq 0$. Esto implica que $\lambda_2^\star=0$, $\lambda_3^\star=0$ y la única restricción activa es $g_1$, tal que
$$
g_1(\xx^\star)=x_1^\star+x_2^\star-1.5=0.
\tag{1.4.11}
$$

Luego, las condiciones KKT se reducen al siguiente sistema, que resulta de (1.4.9) y (1.4.10):
$$
\begin{pmatrix}
1&0&1\\
0&4&1\\
...
Es importante remarcar que la unicidad de la solución es consistente con el hecho de que hemos resuelto un problema de optimización convexa cuya función objetivo es estrictamente convexa.
...
donde el conjunto factible $\Omega$ está definido como una intersección de restricciones funcionales diferenciables. La generalización del análisis previo surge del siguiente hecho:
...
<figure style="text-align: center;">
  <img src="figuras/cono_normal_generalization.svg" alt="" width="350px">
  <figcaption>**Figura 1.4.3:** Cono normal de la linearización de restricciones activas en un punto $\xx_0\in\Omega$.</figcaption>
</figure>

<div style="margin-top:2em;"></div>


Esta idea es más bien intuitiva, por lo cual no estamos en condiciones de extender el Teorema 1.4.7 ni el Corolario 1.4.8 de manera rigurosa. Sin embargo, lo que sí haremos es extender el concepto de las condiciones KKT, dejando para más adelante el análisis de su aplicabilidad. Para ello, utilizaremos los resultados de la sección anterior en $\tilde{\Omega}$, la *linearización* de $\Omega$ en un punto dado. Pero, ¿qué conjunto es $\tilde{\Omega}$?

Sea $\xx^\star\in\Omega$ un punto óptimo y sea $I(\xx^\star)$ el conjunto de índices de las restricciones activas. Para modelar el comportamiento local de $\Omega$ en $\xx^\star$, consideramos las siguientes linearizaciones:
$$
\tilde{g}_i(\xx):=g_i(\xx^\star)+\nabla g_i(\xx^\star)^\top(\xx-\xx^\star)\qquad \forall i\in I(\xx^\star),
$$
$$
\tilde{h}_i(\xx):=h_i(\xx^\star)+\nabla h_i(\xx^\star)^\top(\xx-\xx^\star)\qquad\forall j=1,\ldots,m.
$$

De este modo, la linearización de $\Omega$ queda definida como
$$
\tilde{\Omega}=\left\{\xx\in\RR^p\left|\;\begin{array}{ll}
\; \tilde{g}_i(\xx)\leq 0,& i\in I(\xx^\star)\\
\; \tilde{h}_j(\xx)=0, & j=1,\ldots,m
\end{array}\right.\right\}.
$$

Como puede observarse en la Figura 1.4.3, $\tilde{\Omega}$ es un poliedro que aproxima a $\Omega$ cerca de $\xx^\star$. Los vectores normales a los hiperplanos y a las fronteras de los semiespacios que definen a $\tilde{\Omega}$ están determinados por $\nabla\tilde{g}_i(\xx)$ y $\nabla\tilde{h}_j(\xx)$, los cuales verifican
$$
\nabla \tilde{g}_i(\xx)=\nabla g_i(\xx^\star)\qquad\text{y}\qquad\nabla \tilde{h}_i(\xx)=\nabla h_i(\xx^\star).
$$

La construcción de $\tilde{\Omega}$ nos permite aplicar el Teorema 1.4.7 para caracterizar el cono normal en $\xx^\star$ como sigue:
$$
...
Si bien los teoremas de optimalidad para $\tilde{\Omega}$ no pueden extenderse de forma directa a $\Omega$, la forma en que fue construida esta linearización y la caracterización de su cono normal sugieren que es posible obtener condiciones de optimalidad para $\Omega$ a partir de las condiciones de optimalidad correspondientes a $\tilde{\Omega}$. No obstante, para establecer rigurosamente esta relación necesitamos imponer hipótesis adicionales que garanticen un comportamiento adecuado de las restricciones en $\xx^\star$.

En principio, formalizaremos el concepto de las condiciones KKT y luego analizaremos bajo qué condiciones estas permiten caracterizar la optimalidad de $\xx^\star$.
...
Estamos en condiciones de formalizar el concepto de las condiciones KKT. Sin embargo, aún no vamos a podemos afirmar que se tratan de condiciones de optimalidad. 
...
::: {.definicion}
**Definición 1.4.12** (Condiciones de Karush-Kuhn-Tucker (KKT)).
Sea un problema de optimización con función objetivo diferenciable y restricciones funcionales, de la forma general (1.4.12). Las *condiciones KKT* en un punto $\xx\in\RR^p$ están dadas por:
...
Hasta ahora tenemos claro que, si el conjunto factible es un poliedro, las condiciones KKT se cumplen en los puntos óptimos. Sin embargo, esto no siempre es cierto en condiciones generales; hay situaciones en las que un punto óptimo no satisface las condiciones KKT. En otras palabras:
...
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
Falla de condiciones KKT en un punto óptimo
</span>
...
<figure style="text-align: center;">
  <img src="figuras/ej_KKT_fail.svg" alt="..." width="400px">
</figure>
...
Claramente, no hay manera de encontrar multiplicadores $\lambda_1$ y $\lambda_3$ que verifiquen la condición de estacionariedad
$$
...
- **Restricciones afines**: Esto lo hemos estudiado como caso particular en la sección anterior, ya que el conjunto factible $\Omega$ es un poliedro. Por lo tanto, no hace falta imponer cualificaciones adicionales.
...
$$
\exists\xx_0\in\relinterior\mathcal{D}:g_i(\xx_0)<0\;\land\; h_j(\xx_0)=0\qquad\forall i=1,\ldots,r,\;\forall j=1,\ldots,m,
$$
entonces las condiciones KKT se cumplen para $\xx^\star$.

  <div class="alert alert-light text-dark" role="important">

  La aparición de $\relinterior\mathcal{D}$ permite que el punto estrictamente factible no se encuentre en el borde relativo del dominio y, por tanto, que sea posible realizar pequeñas perturbaciones en cualquier dirección. Esto resulta particularmente importante cuando $\mathcal{D}$ está contenido en un subespacio afín de $\mathbb{R}^p$ y, por lo tanto, tiene interior vacío. Cuando $\mathcal{D}$ tiene interior no vacío, se tiene $\relinterior\mathcal{D}=\interior\mathcal{D}$.
...
Si no hay restricciones de desigualdad y las restricciones de igualdad son afines, la condición de Slater se reduce a la existencia de un punto factible en $\relinterior\mathcal{D}$. En consecuencia, en problemas de la forma
$$
\begin{array}{ll} 
	\text{minimizar } & f(\xx)\\ 
	\text{sujeto a } & A\xx=\bb,
...
con $\dom f=\RR^p$, la existencia de un punto factible es suficiente para garantizar que las condiciones KKT se satisfacen en todo punto óptimo.
...
::: {.teorema}
**Teorema 1.4.14** (Caracterización de optimalidad mediante KKT). 
Sea un problema de optimización convexa de la forma
$$ 
	\begin{array}{ll} 
		\text{minimizar } & f(\xx)\\ 
		\text{sujeto a }  & g_i(\xx)\leq 0,\qquad i=1,\cdots,r,\\
			 			  & A\xx=\bb,
	\end{array}
$$ 
con $A\in\RR^{m\times p}$ y $\bb\in\RR^m$. Si la función objetivo $f$ y las funciones de restricción $g_1,\ldots, g_r$ son diferenciables, y además las restricciones verifican la condición de Slater, entonces las condiciones KKT son suficientes y necesarias para que un punto sea óptimo.
:::
...
Por condición de optimalidad de primer orden sin restricciones para funciones convexas (Teorema 1.4.1), la expresión (1.4.13) implica que $\xx_0$ es el minimizador de $L(\xx_0)$. Además, es fácil ver que $L(\xx)\leq f(\xx)$ para todo $\xx$ factible. En consecuencia, se deduce
$$
...
$$
\lambda_i g_i(\xx_0)=0\qquad\forall i=1,\ldots,r.
$$

Luego, $\xx_0$ es punto óptimo del problema, tal como queríamos probar.
<div style="margin-top:-1em;"></div>
<div style="text-align: right;">$\blacksquare$</div>
...
::: {.callout .question}
<span style="font-size: 1.3em;">✍️</span><br>
En la demostración anterior, justifique que $L$ es una función convexa, $L(\xx)\leq f(\xx)$ para todo $\xx$ factible y $L(\xx_0)=f(\xx_0)$.
...
<span class="badge bg-primary">Ejemplo 1.4.15</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
Condiciones KKT suficientes y necesarias para optimalidad
</span>
...
- Por condición de estacionariedad, $\lambda_1^\star<0$, lo cual contradice la factibilidad dual.
...
## III <span style="color:#CCCCCC;">│</span> Dualidad

La teoría de la dualidad es fundamental en optimización, pues permite reformular problemas mediante funciones duales que pueden proporcionar información valiosa sobre la solución óptima.

### Función dual de Lagrange

::: {.definicion}
**Definición 1.4.16** (Lagrangiano). 
Sea un problema de optimización de la forma general (1.4.12). Se denomina *Lagrangiano* a la función $\calL:\RR^p\times\RR^r\times\RR^m\to\RR$ definida por
$$
\calL(\xx,\bflambda,\bfnu):=f(\xx)+\sum_{i=1}^r\lambda_i g_i(\xx)+\sum_{j=1}^m\nu_j h_j(\xx),
$$

con $\dom\calL:=\mathcal{D}\times\RR^r\times\RR^m$. Las componentes de $\bflambda$ y $\bfnu$ se denominan *multiplicadores de Lagrange*.
:::

<div style="margin-top:1.5em;"></div>

En adelante, nos referiremos al problema de optimización general como *problema primal*. A partir del Lagrangiano, construimos a continuación una función que luego nos permitirá derivar una cota inferior para el valor óptimo $p^\star$ del problema primal.

<div style="margin-top:1em;"></div>


::: {.definicion}
**Definición 1.4.17** (Función dual de Lagrange). 
Sea $\calL$ el Lagrangiano de la Definición 1.4.16. Se denomina *función dual de Lagrange* a $\calG:\RR^r\times\RR^m\to\RR$ definida por
$$
...
Observar que, para un $\xx$ fijo, el Lagrangiano es una función afín de $(\bflambda,\bfnu)$. Por lo tanto, la función dual $\calG$ es el ínfimo puntual de una familia de funciones afines. Como consecuencia, de acuerdo con las operaciones que preservan convexidad vistas en [C1-S2], podemos afirmar que $\calG$ *es siempre una función cóncava*, lo cual será fundamental más adelante.
...
::: {.callout .question}
<span style="font-size: 1.3em;">✍️</span><br>
Justifique la propiedad anterior.
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 1.4.18</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
  Solución de mínima norma de un sistema de ecuaciones lineales (parte 1)
</span>

Sean $A\in\RR^{m\times p}$ y $\bb\in\RR^m$. Consideremos el problema
...
Dado que $\calL$ es una función convexa, podemos minimizar respecto de $\xx$ usando la condición de optimalidad $\nabla_{\xx}\calL(\xx,\bfnu)=\bfzero$. Resulta
$$
\nabla_{\xx}\calL(\xx,\bfnu)=2\xx+A^\top\bfnu=\bfzero\quad\rightarrow\quad \xx=-\frac{1}{2}A^\top\bfnu.
$$
...
Dado que $\calG$ es una función cóncava, maximizarla para $\bflambda\succeq\bfzero$ conduce a un problema de optimización convexo, independientemente de si el problema primal lo es o no. 


::: {.definicion}
**Definición 1.4.19** (Problema dual de Lagrange). 
Sea $\calG$ la función dual de Lagrange asociado al problema primal. El *problema dual de Lagrange* es
$$
\begin{array}{ll}
...
Dado que $\calG$ es una función cóncava, podemos afirmar que:
...
<span class="badge bg-success text-light">Dualidad débil</span>

El valor óptimo $d^\star$ del problema dual es la mejor cota inferior de $p^\star$ que podemos obtener mediante la función dual de Lagrange. La desigualdad
$$
d^\star\leq p^\star
\tag{1.4.14}
$$
se conoce como *dualidad débil*, y es cierta incluso si el problema primal no es convexo. También podemos afirmar que:
...
Nos referiremos a la diferencia $p^\star-d^\star$ como *brecha de dualidad óptima* del problema primal. Es importante remarcar que la ventaja de trabajar con la dualidad débil radica en que, en ocasiones, permite obtener eficientemente cotas inferiores del valor óptimo primal.
...
<div class="alert alert-light text-dark" role="alert">
<span class="badge bg-success text-light">Dualidad fuerte</span>

La *dualidad fuerte* se cumple si ocurre 
$$
d^\star=p^\star.
\tag{1.4.15}
$$
...
::: {.teorema}
**Teorema 1.4.20** (Teorema de Slater). 
Sea un problema de optimización convexa de la forma
$$ 
\begin{array}{ll} 
	\text{minimizar } & f(\xx)\\ 
...
$$
\exists\xx_0\in\relinterior \mathcal{D}:g_i(\xx_0)<0\;\forall i=1,\ldots,r\;\land\; A\xx_0=\bb,
$$
...
::: {.callout .question}
<span style="font-size: 1.3em;">✍️</span><br>
Utilice el siguiente ejemplo para mostrar que Slater no es una condición necesaria para la dualidad fuerte.
...
<span class="badge bg-primary">Ejemplo 1.4.21</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
  Solución de mínima norma de un sistema de ecuaciones lineales (parte 2)
</span>

Consideremos el problema del Ejemplo 1.4.18, con $m=1$:
$$
\begin{array}{ll}
\text{minimizar } & \xx^\top\xx\\
\text{sujeto a }  & \bfa^\top\xx=b,
\end{array}
$$
con $\bfa\in\RR^p$ y $b\in\RR$. Tenemos un solo multiplicador de Lagrange, $\nu\in\RR$, y la función dual es
$$
\calG(\nu)=-\frac{1}{4}\|\bfa\|_2^2\nu^2-b\nu.
$$
...
$$
\begin{array}{ll}
\text{maximizar } & -\frac{1}{4}\|\bfa\|_2^2\nu^2-b\nu\\
\text{sujeto a }  & \nu>0.
\end{array}
$$

Este es un problema fácil de resolver, porque $\calG(\nu)$ es una función cuadrática de una variable, de manera que la solución está dada por su vértice. Resulta
$$
\nu^\star=-\frac{2b}{\|\bfa\|_2^2}\qquad\text{tal que}\qquad d^\star=\calG(\nu^\star)=\frac{b^2}{\|\bfa\|_2^2}.
$$

Dado que se cumplen las condiciones del Teorema 1.4.20 (problema de optimización convexa + condición de Slater), podemos concluir que hay dualidad fuerte. Luego, 
$$
p^\star=\frac{b^2}{\|\bfa\|_2^2}.
$$

Observar que esto muestra que la distancia del hiperplano $\bfa^\top\xx=b$ al origen es $b/\|\bfa\|_2$, tal como lo mencionamos en [C1-S2](A2_optimizacion_convexa.html).
:::
...
Hay que tener cuidado al interpretar la expresión (1.4.16): si bien garantiza que $f(\xx^\star)$ es el valor ínfimo del Lagrangiano, no nos dice nada acerca de cuál es el valor de $\xx$ donde dicho ínfimo se alcanza, y en principio solo podemos escribir
$$
f(\xx^\star)\leq\calL(\xx^\star,\bflambda^\star,\bfnu^\star)
$$

No obstante, la naturaleza de los multiplicadores de Lagrange y las restricciones funcionales nos permiten agregar
$$
f(\xx^\star)\leq \calL(\xx^\star,\bflambda^\star,\bfnu^\star)\\
...
$$
f(\xx^\star)=\calL(\xx^\star,\bflambda^\star,\bfnu^\star)=f(\xx^\star)+\sum_{i=1}^r\lambda_ig_i(\xx^\star).
\tag{1.4.17}
$$

En consecuencia, a partir de (1.4.16) y (1.4.17) podemos concluir que, bajo dualidad fuerte, la terna $(\xx^\star,\bflambda^\star,\bfnu^\star)$ verifica dos condiciones KKT de la Definición 1.4.12: holgura complementaria y estacionariedad. En efecto:

- *Holgura complementaria*: para que la igualdad en (1.4.17) sea cierta, debe ser
  $$
...
- *Estacionariedad*: el ínfimo de $\calL(\xx,\bflambda^\star,\bfnu^\star)$ sí ocurre en $\xx^\star$:
  $$
  ...
  La suposición de que la función objetivo y las funciones de restricción son diferenciables garantiza que el Lagrangiano también lo es. Además, si las funciones involucradas están definidas en conjuntos abiertos, podemos asegurar que $\dom\calL:=\mathcal{D}\times\RR^r\times\RR^m$ es un conjunto abierto de $\RR^{p+r+m}$. Así, $\xx^\star$ es un punto interior de $\dom\calL$ y, por lo tanto, la condición de optimalidad de primer orden asegura que
  $$
...
::: {.teorema}
**Teorema 1.4.22** (Dualidad y KKT). 
Sea $\xx^\star$ un punto óptimo primal y $(\bflambda^\star,\bfnu^\star)$ un punto óptimo dual. Si $d^\star=p^\star$, entonces la terna $(\xx^\star,\bflambda^\star,\bfnu^\star)$ satisface las condiciones de Karush-Kuhn-Tucker.
:::
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 1.4.23</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
$$
\sum_{i=1}^p x_i=\bfone^\top\xx=1,\qquad x_i\geq 0.
$$
...
$$
\begin{array}{ll}
\text{minimizar } & -\displaystyle\sum_{i=1}^p\log(x_i+\alpha_i)\\[3pt]
\text{sujeto a }  & \mathbf{1}^\top\xx=1,\\
                  & \xx\succeq \bfzero. 
\end{array}
$$

Observar que hay $p$ restricciones de desigualdad, con $g_i(\xx)=-x_i$, y una única restricción de igualdad, con $h(\xx)=\mathbf{1}^\top\xx-1$. Claramente la condición de Slater se cumple: podemos tomar, por ejemplo, el punto estrictamente factible $x_i=1/p$ (para todo $i=1,\ldots,p$). Por lo tanto, podemos asegurar que se cumple la dualidad fuerte y que las condiciones KKT son suficientes y necesarias para la optimalidad.
...
El análisis de las últimas dos, junto con $\xx^\star\succeq\bfzero$, permiten deducir que
$$
x_i^\star=\left\{\begin{array}{cl}
1/\nu^\star-\alpha_i&\mbox{si }\nu^\star<\frac{1}{\alpha_i},\\
0&\mbox{si }\nu^\star\geq\frac{1}{\alpha_i}.
\end{array}\right. = \max\{0,1/\nu^\star-\alpha_i\}.
$$ 

Finalmente, el uso de la condición $\mathbf{1}^\top\xx^\star=1$ resulta en
$$
\sum_{i=1}^p\max\{0,1/\nu^\star-\alpha_i\}=1.
\tag{1.4.18}
$$

El lado izquierdo de (1.4.18) corresponde a una función de $1/\nu^\star$ que es continua, creciente por tramos y con puntos de quiebre en los valores $\alpha_i$, tal y como lo ejemplifica la siguiente figura:

<figure style="text-align: center;">
  <img src="figuras/ej_water_filling_1.svg" alt="Texto" width="470px">
</figure>
...
<figure style="text-align: center;">
  <img src="figuras/ej_water_filling_2.svg" alt="Texto" width="400px">
  <figcaption>**Figura 1.4.4:** Método de *water-filling*: la región se inunda hasta un nivel $1/\nu^\star$, utilizando una cantidad total de agua igual a 1.</figcaption>
</figure>
...
Finalizamos esta sección con un cuadro resumen con los principales resultados.

<div style="margin-top:2em;"></div>

<figure style="text-align: center;">
  <img src="figuras/cuadro_resumen_A4.svg" alt="Texto" width="600px">
</figure>
