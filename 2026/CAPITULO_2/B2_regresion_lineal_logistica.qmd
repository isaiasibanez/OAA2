...
- $\mathbb{P}(A)$ la probabilidad de que ocurra un suceso $A$.
- Para una variable aleatoria $Z\in\RR$, se denota $\media\left[Z\right]$ su valor esperado y $\var\left[Z\right]$ su varianza. 
- La covarianza entre dos variables $Z_1$ y $Z_2$ se denota $\cov\left[Z_1,Z_2\right]$. Por supuesto, se cumple $\var\left[Z\right]=\cov\left[Z,Z\right]$.
...
Analice qué sucede al aplicar el método de Newton para resolver el problema de regresión lineal.
...
$$
\plim_{n\to\infty}\hat{\bftheta}_n=\bftheta^\star\qquad (\hat{\bftheta}_n\xrightarrow{p}\bftheta^\star).
$$
