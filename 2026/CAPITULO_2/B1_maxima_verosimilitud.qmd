...
Durante esta sección consideramos una muestra i.i.d. $\{y_1, \ldots, y_n\}$ de una variable aleatoria $Y\in\RR$ cuya distribución depende de un vector de parámetros $\bftheta \in \RR^q$. Denotamos con $p(y \mid \bftheta)$ la función de densidad (o masa de probabilidad) de $Y$.

Además, de ahora en adelante, denotamos $\mathbb{P}(A)$ la probabilidad de que ocurra un suceso $A$ y, para una variable aleatoria $Z$, se denota $\media\left[Z\right]$ su valor esperado y $\var\left[Z\right]$ su varianza.
...
::: {.definicion}
**Definición 2.1.2** (Estimador de máxima verosimilitud). Sea $\{y_1,\ldots,y_n\}$ una muestra i.i.d. de una variable aleatoria $Y\in\RR$, y sea $p(\cdot\mid\bftheta)$ un modelo parametrizado por $\bftheta\in\RR^q$. Se denomina *estimador de máxima verosimilitud* (MLE) a cualquier punto óptimo del problema (2.1.2), a saber:
$$
\hat{\bftheta}_{\text{MLE}}\in\arg\max_{\bftheta\in\Theta}\;L(\bftheta).
$$
...
Si el modelo está correctamente especificado, el MLE $\hat{\bftheta}$ constituye una estimación muestral del *parámetro poblacional*, que denotaremos por $\bftheta_\star$.
...
Así, el problema (2.1.2) es equivalente a
$$
\begin{array}{ll}
...
Dado que $\ell$ es estrictamente cóncava, ya que
$$
\ell''(\phi)=-\frac{n\bar{y}}{\phi^2}-\frac{n(1-\bar{y})}{(1-\phi)^2}<0\qquad\forall\, 0<\phi<1,
$$

la ecuación de verosimilitud (2.1.4) es suficiente y necesaria para la optimalidad. Para (2.1.5), resolvemos $\ell'(\phi)=0$:
$$
\begin{align*}
...
Dado que $\hat{\bftheta}$ es un estimador de $\bftheta_\star$, resulta relevante estudiar sus propiedades estadísticas, tales como consistencia, sesgo y varianza. Recordemos que la *consistencia* mide su convergencia a $\bftheta_\star$, el *sesgo* su error sistemático y la *varianza* su variabilidad entre muestras.
...
- **Invarianza**. Si $\hat{\bftheta}$ es el MLE de $\bftheta$ y $g$ es una función de $\bftheta$, entonces $g(\hat{\bftheta})$ es el MLE de $g(\bftheta)$.
...
- **Normalidad asintótica**. Bajo condiciones de regularidad,
  ...
  es la *información de Fisher* de una observación, que cuantifica cuánta información sobre $\bftheta$ se espera obtener de ella si el parámetro toma dicho valor. La normalidad asintótica permite construir intervalos de confianza y tests de hipótesis de forma sistemática a partir del MLE.
...
En problemas suficientemente complejos, estas propiedades pueden dejar de cumplirse y el MLE puede dejar de ser un buen estimador. Las *condiciones de regularidad* a las que se hacen referencia son, esencialmente, condiciones de suavidad sobre $p(y\mid \bftheta)$ que garantizan un comportamiento adecuado del modelo y de la función de verosimilitud. Salvo que se indique lo contrario, supondremos implícitamente que estas condiciones se cumplen.
...
Sea $\{y_1,\ldots,y_n\}$ una muestra i.i.d. de $Y\sim\text{Bernoulli}(\varphi)$, con $0<\phi<1$. Supongamos que el modelo está correctamente especificado, siendo $\phi_\star$ es el parámetro poblacional. Del Ejemplo 2.1.3, tenemos que
$$
...
Por lo tanto, la cota de Cramér-Rao establece que, para cualquier estimador insesgado $\tilde{\phi}$ de $\phi_\star$, se verifica
$$
\var\left[\tilde{\phi}\right]\geq \frac{\phi_\star(1-\phi_\star)}{n}
$$
	
En particular, el MLE $\hat{\phi}$ alcanza esta cota para cualquier tamaño muestral $n$, ya que
$$
\var\left[\hat{p}\right]=\var\left[\frac{1}{n}\sum_{i=1}^n Y_i\right]=\frac{1}{n^2}\sum_{i=1}^n\var\left[Y_i\right]=\frac{\phi_{\star}(1-\phi_{\star})}{n},
$$
	
donde hemos usado el hecho que $\{Y_1,\ldots,Y_n\}$ son i.i.d. y que, bajo el modelo especificado, $\var\left[Y_i\right]=\phi_{\star}(1-\phi_{\star})$.
