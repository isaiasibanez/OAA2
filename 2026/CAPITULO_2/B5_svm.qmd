...
<a href="../material/apunte_c2s5.pdf" target="_blank" class="btn btn-outline-primary">
<i class="fa-solid fa-file-lines me-2"></i>Versión PDF
</a>

<a href="../material/tp_c2s5.pdf" target="_blank" class="btn btn-outline-success">
...
Una característica distintiva de estos métodos es que su desarrollo se apoya fuertemente en la formulación dual de problemas de optimización y en las condiciones de Karush-Kuhn-Tucker (KKT), lo que permite interpretar la solución en términos de los vectores soporte y comprender el funcionamiento del método.

Durante esta sección, vamos a considerar un problema de clasificación binaria. Sin pérdida de generalidad, supondremos que el conjunto de datos $\{(\xx_i,y_i)\}_{i=1}^n$ tiene etiquetas $y_i\in\{-1,+1\}$.

<div style="margin-top:2.5em;"></div>



## I <span style="color:#CCCCCC;">│</span> Hiperplano separador óptimo
...
::: {.definicion}
**Definición 2.5.1.** (Separabilidad lineal) Un conjunto de datos $\{(\xx_i,y_i)\}_{i=1}^n$, con $y_i\in\{-1,+1\}$, se dice que es *linealmente separable* si existen $\bfbeta\in\RR^p$ y $\beta_0\in\RR$ tales que
$$
y_i=\sign(\bfbeta^\top\xx_i+\beta_0)\qquad \forall i=1,\ldots,n.
\tag{2.5.1}
$$
...
<figure style="text-align: center;">
  ...
  <figcaption> **Figura 2.5.1**. Datos linealmente separables y algunos hiperplanos separadores. </figcaption>
</figure>

<div style="margin-top:2em;"></div>

Observar que la expresión (2.5.1) es equivalente a requerir
$$
y_i(\bfbeta^\top\xx_i+\beta_0)>0\qquad\forall i=1,\ldots, n.
\tag{2.5.2}
$$

Algo intuitivo (ver Figura 2.5.1) es que si las clases son totalmente separables, existen infinitos hiperplanos separadores. Por lo tanto, resulta necesario definir un criterio que permita seleccionar un hiperplano único y óptimo. 

En primer lugar, establecemos lo siguiente:
...
Este primer principio justifica considerar a $\bfbeta\in\RR^p$ como variable de optimización, ya que establece una correspondencia biunívoca entre cada vector $\bfbeta$ y un hiperplano separador, ¿cuál? Elegimos aquel que *equidista* de ambas clases. Para caracterizar esta propiedad, primero necesitamos imponer una restricción de normalización, ya que la distancia de un punto al hiperplano depende de $\|\bfbeta\|_2$.
...
Así, bajo este segundo principio, la distancia entre cualquier punto $\xx_i$ y el hiperplano $\bfbeta^\top\xx+\beta_0=0$ está dada por
$$
|f(\xx_i)|=|\bfbeta^\top\xx_i+\beta_0|=y_i(\bfbeta^\top\xx_i+\beta_0).
$$


Con estas consideraciones, ya podemos definir el hiperplano separador asociado a un vector $\bfbeta\in\RR^p$. Esta definición determina el valor del intercepto $\beta_0$, completando así la caracterización del hiperplano.

<div style="margin-top:1em;"></div>

::: {.definicion}
**Definición 2.5.2.** (Hiperplano separador asociado a un vector) A un vector $\bfbeta\in\RR^p$ se le asocia el hiperplano separador $\bfbeta^\top\xx+\beta_0=0$ cuya distancia mínima a ambas clases es la misma. Es decir, existe $M\in\RR^+$ tal que 
$$
y_i(\bfbeta^\top\xx_i+\beta_0)\geq M\qquad\forall i=1,\ldots,n,
$$
con igualdad para al menos un punto de cada clase. Al valor $M$ lo denominamos *margen* del hiperplano separador asociado a $\bfbeta$.
:::

<div style="margin-top:2em;"></div>

Es importante observar que los puntos para los cuales se cumple la igualdad en la restricción son los que caracterizan al hiperplano; estos se denominan <mark>vectores soporte</mark> (su definición formal se presentará más adelante).
...
<figure style="text-align: center;">
  ...
  <figcaption> **Figura 2.5.2**. Hiperplanos separadores y sus margenes asociados. </figcaption>
</figure>

<div style="margin-top:2em;"></div>

Ya estamos en condiciones de determinar qué consideraremos como hiperplano separador óptimo de un conjunto de datos linealmente separables.

::: {.definicion}
**Definición 2.5.3.** (Hiperplano separador óptimo) Sea $\{(\xx_i,y_i)\}_{i=1}^n$, con $y_i\in\{-1,+1\}$, un conjunto de datos linealmente separable. El *hiperplano separador óptimo* es aquel que maximiza el *margen* $M$.
...
Observar que las variables de optimización son $\bfbeta\in\RR^p$ y $\beta_0\in\RR$. Si bien esta formulación refleja directamente la intención de maximizar el margen, vamos a ajustarla para obtener una formulación clásica más conveniente.
...
En base a lo descripto anteriormente, la restricción $\|\bfbeta\|_2=1$ permite interpretar al margen $M$ como la distancia mínima de los puntos al hiperplano. Si se relaja dicha condición, dicha interpretación sigue siendo cierta si se escribe
$$
\frac{y_i(\bfbeta^\top\xx_i+\beta_0)}{\|\bfbeta\|_2}\geq M,\qquad i=1,\ldots,n.
$$

En consecuencia, el problema (2.5.3) se puede reescribir como
$$
\begin{array}{ll}
\text{maximizar } & M\\
...
\tag{2.5.4}
$$
con una importante diferencia: dado un punto óptimo $\bfbeta^\star$ de (2.5.4), también lo son los puntos $k\bfbeta^\star$, con $k\in\RR^+$. Es decir, al eliminar la restricción $\|\bfbeta\|_2=1$, la elección del vector normal asociado a un hiperplano vuelve a ser arbitraria. Por tal motivo, elegiremos nuevamente un representante entre todos los múltiplos escalares de un mismo vector normal, aunque esta vez la elección dependerá del valor del margen $M$. 


::: {.myhighlight}

Entre todos los puntos óptimos del problema (2.5.4) que representan el mismo hiperplano separador, elegimos el único que verifica
$$
\|\bfbeta^\star\|_2=\frac{1}{M^\star},
$$
donde $M^\star$ es el margen óptimo.

:::

<div style="margin-top:2em;"></div>

Esta idea nos va a permitir reformular nuestro problema de optimización. En efecto, bajo estas nuevas condiciones, maximizar $M$ es equivalente a minimizar $\|\bfbeta\|_2$ y, convenientemente,  también equivalente a minimizar $\frac{1}{2}\|\bfbeta\|_2^2$. Finalmente:

::: {.definicion}

**Definición 2.5.4.** (Problema de optimización del hiperplano separador óptimo) Dado un conjunto de datos  $\{(\xx_i,y_i)\}_{i=1}^n$ linealmente separables, el hiperplano separador óptimo es 
$$
...
$$
\begin{array}{ll}
\text{minimizar } & \displaystyle\frac{1}{2}\|\bfbeta\|_2^2\\
...
::: {.myhighlight}
El problema (2.5.5) es un problema de optimización convexa cuyo conjunto factible es un poliedro.
:::

<div style="margin-top:2em;"></div>

En consecuencia, el Corolario 1.4.8 del [C1-S4](A4_condiciones_optimalidad.html#el-caso-particular-de-restricciones-afines) nos asegura que las condiciones de Karush-Kuhn-Tucker (KKT) son suficientes y necesarias para la optimalidad. Más aún, el Teorema 1.4.19 de esa misma sección (Teorema de Slater) permite afirmar que hay dualidad fuerte.
...
El Lagrangiano asociado al problema (2.5.5) es $\calL:\RR^{p+1}\times\RR^n\to\RR$ definido por
$$
\calL(\bfbeta,\beta_0,\bflambda)=\frac{1}{2}\|\bfbeta\|_2^2-\sum_{i=1}^n\lambda_i\left[y_i(\bfbeta^\top\xx_i+\beta_0)-1\right].
$$

Dado que el Lagrangiano es una función convexa en $(\bfbeta,\beta_0)$, la ecuación $\nabla_{(\bfbeta,\beta_0)}\calL=\bfzero$ caracteriza el ínfimo de $\calL$ para cada $\bflambda$. Esto resulta en las expresiones 
$$
\bfbeta=\sum_{i=1}^n\lambda_i y_i \xx_i,\qquad \sum_{i=1}^n\lambda_i y_i=0.
$$

Observar que $\bfbeta$ queda definida en términos de $\bflambda$ y, aunque $\beta_0$ no queda determinado de forma directa, sí nos provee una restricción de consistencia sobre los multiplicadores. Reemplazando, obtenemos la función dual de Lagrange:
$$
\begin{align}
\calG(\bflambda):&=\inf_{\bfbeta,\beta_0}\calL(\bfbeta,\beta_0,\bflambda)\\
&=\frac{1}{2}\left\|\sum_{i=1}^n\lambda_iy_i\xx_i\right\|_2^2-\sum_{i=1}^n\lambda_iy_i\left(\sum_{j=1}^n\lambda_jy_j\xx_j\right)^\top\xx_i-\beta_0\underbrace{\sum_{i=1}^n\lambda_iy_i}_{=0}+\sum_{i=1}^n\lambda_i\\
...
Observar, en la primera igualdad de (2.5.6), el término 
$$
-\beta_0\sum_{i=1}^n\lambda_i y_i.
$$

Esto implica que $\calG(\bflambda)=-\infty$ si $\sum_{i=1}^n\lambda_i y_i\neq 0$. Por lo tanto, para el problema dual, que pretende maximizar (2.5.6), solo tienen sentido los $\bflambda$ que cumplen justamente la restricción
$$
\sum_{i=1}^n\lambda_i y_i=0.
$$

Incluir esta condición como restricción en el problema dual permite descartar automáticamente del conjunto factible los $\bflambda$ que no la cumplen. Además, favorece la simplificación de la función objetivo, tal como lo hicimos.
...
Para obtener el hiperplano separador óptimo de un conjunto de datos linealmente separable, los pasos a seguir son los siguientes:

1. Se resuelve el problema dual (2.5.6) para obtener $\bflambda^\star$.
...
$$
donde $\mathcal{S}=\{i\mid \lambda_i^\star\neq 0\}$. Aquí, cada término se obtiene al despejar $\beta_0^\star$ de la condición de holgura complementaria para los puntos que verifican $\lambda_i^\star\neq 0$.
...
#### Interpretación de condiciones KKT

Pongamos foco en la factibilidad primal y en la holgura complementaria:
$$
\begin{array}{cl}
y_i((\bfbeta^\star)^\top\xx_i+\beta_0^\star)-1\geq 0\qquad\forall i=1,\ldots,n\qquad&\text{(Factibilidad primal)}\\[3pt]
\lambda_i^\star\left[y_i((\bfbeta^\star)^\top\xx_i+\beta_0^\star)-1\right]=0\qquad\forall i=1,\ldots,n\qquad&\text{(Holgura complementaria)}\\[3pt]
...
De la condición de holgura complementaria, podemos afirmar que
$$
\lambda_i^\star>0 \Rightarrow y_i((\bfbeta^\star)^\top\xx_i+\beta_0^\star)=1,
\tag{2.5.8}
$$
lo cual significa que $\xx_i$ está sobre el *borde* del margen. Esto motiva la siguiente definición.

<div style="margin-top:1em;"></div>


::: {.definicion}
**Definición 2.5.5.** (Vector soporte) Un dato $\xx_i$ se denomina *vector soporte* si el multiplicador asociado $\lambda_i$ es estrictamente positivo. 
...
De la solución $\bfbeta^\star$ obtenida previamente, queda claro que los vectores soportes son quienes caracterizan el hiperplano separador óptimo. Además:

- La recíproca de la implicación (2.5.8) no es cierta. Es decir, que un punto esté sobre el borde del margen no garantiza que su multiplicador sea positivo. Esto puede ocurrir cuando hay redundancia lineal entre las restricciones activas (más puntos sobre el margen que los estrictamente necesarios para determinar el hiperplano), en cuyo caso el problema dual admite múltiples soluciones óptimas. En la práctica, este fenómeno dificilmente ocurre.

- Combinando la factibilidad primal con la holgura complementaria, se sigue que los puntos que no están en el borde necesariamente tienen multiplicador nulo. Por lo tanto, no pueden ser vectores soporte y no contribuyen a la solución del problema.
...
<figure style="text-align: center;">
  ...
  <figcaption> **Figura 2.5.3**. Vectores soporte de un hiperplano separador óptimo. </figcaption>
</figure>
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 2.5.6</span> 
...
## II <span style="color:#CCCCCC;">│</span> Variables de holgura y margen suave

En la formulación del hiperplano separador óptimo, se asume que el conjunto de datos $\{(\xx_i,y_i)\}_{i=1}^n$ es linealmente separable. Sin embargo, en la práctica lo habitual es que exista solapamiento entre las clases. Para extender el método, la estrategia consiste en introducir *variables de holgura*, simbolizadas con $\boldsymbol{\xi}\in\RR^n$, que permitan que algunos puntos violen el margen o incluso queden mal clasificados. Este enfoque, conocido como *margen suave* (*soft-margin*, en inglés), es lo que desarrollaremos a continuación.

<figure style="text-align: center;">
  <img src="figuras/svm_holgura.svg" alt="…" width="550px">
  <figcaption> **Figura 2.5.4**. Variables de holgura. </figcaption>
</figure>


La incorporación de las variables de holgura $\bfxi$ al problema de optimización (2.5.5) conduce al siguiente problema de optimización:
$$
\begin{array}{ll}
\text{minimizar } & \displaystyle\frac{1}{2}\|\bfbeta\|_2^2\\
...
::: {.myhighlight}
El problema (2.5.9) es un problema de optimización convexa cuyo conjunto factible es un poliedro.
:::

<div style="margin-top:2em;"></div>


Estamos en condiciones de aplicar nuevamente las condiciones de Karush-Kuhn-Tucker (KKT) y la dualidad para caracterizar la solución de este problema. De ahora en adelante, nos referiremos a este nuevo método como <mark>Máquina de Vectores Soporte (SVM)</mark> lineal.
...
En el problema de optimización (2.5.9) se distinguen $2n$ restricciones de desigualdad. Para aquellas asociadas a las variables de holgura, $\bfxi\succeq 0$, distinguiremos sus multiplicadores con la notación $\bfmu=(\mu_1,\ldots,\mu_n)$. Las condiciones KKT son
$$
\begin{array}{cl}
...
Hemos obtenido la misma función dual que para el caso sin variables de holgura. Sin embargo, el problema dual no es el mismo, puesto que hay que considerar la restricción $\bfmu\succeq \bfzero$:
$$
\begin{array}{ll}
\text{maximizar } & \displaystyle\sum_{i=1}^n\lambda_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^n\lambda_i\lambda_j y_iy_j\xx_j^\top\xx_i\\
...
Sin embargo, podemos combinar las restricciones de negatividad, en virtud de la condición de estacionariedad $C=\lambda_i+\mu_i$, y escribir en (2.5.10) simplemente $0\leq \lambda_i\leq C$ para todo $i=1,\ldots,n$. Esto deriva en la versión final del problema dual:
$$
\begin{array}{ll}
\text{maximizar } & \displaystyle\sum_{i=1}^n\lambda_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^n\lambda_i\lambda_j y_iy_j\xx_j^\top\xx_i\\
...
Para obtener el hiperplano óptimo de margen suave de un conjunto de datos, los pasos a seguir son los siguientes:

1. Se resuelve el problema dual (2.5.11) para obtener $\bflambda^\star$.
...
\beta_0^\star=\frac{1}{|\mathcal{S}|}\sum_{i\in\mathcal{S}} \left(y_i-(\bfbeta^\star)^\top\xx_i\right),
$$
donde $\mathcal{S}=\{i\mid \lambda_i^\star\neq 0\land\xi_i^\star=0\}$. Aquí, cada término se obtiene al despejar $\beta_0^\star$ de la condición de holgura complementaria para los puntos que verifican $\lambda_i^\star\neq 0$ y $\xi_i^\star=0$.
...
En este caso, la holgura complementaria conduce a
$$
\lambda_i^\star>0 \Rightarrow
y_i((\bfbeta^\star)^\top\xx_i+\beta_0^\star)=1-\xi_i^\star,
$$

lo cual implica dos opciones para los vectores soporte:
...
<figure style="text-align: center;">
  ...
  <figcaption> **Figura 2.5.5**. Vectores soporte de un hiperplano óptimo con margen suave. </figcaption>
</figure>

<div style="margin-top:2em;"></div>


En este caso, la lectura combinada de la factibilidad primal y la holgura complementaria es: si $y_i((\bfbeta^\star)^\top\xx_i+\beta_0^\star)>1-\xi_i^\star$, entonces debe ser $\lambda_i^\star=0$. En este momento la pregunta de interés es:
...
Si $\xi_i^\star>0$, la condición de holgura complementaria $\mu_i^\star\xi_i^\star=0$ junto a la condición de estacionariedad $C=\lambda_i^\star+\mu_i^\star$ nos permiten deducir que 
$$
\lambda_i^\star=C.
$$
...
A continuación, analicemos las posibilidades para un punto $\xx_i$ con multiplicador $\lambda_i^\star$.
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 2.5.7</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
## III <span style="color:#CCCCCC;">│</span> Máquinas de vectores soporte no lineal
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 2.5.8</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
...
El ejemplo anterior motiva la idea de usar transformaciones no lineales para mejorar las condiciones de un problema. Dicha transformación debe ser de la forma
$$
\phi:\RR^p\to\calH,
$$

donde $\calH$ es un *espacio de Hilbert*; es decir, un espacio donde tenemos un producto interno $\langle\cdot,\cdot\rangle_{\calH}$ definido. La condición de que $\calH$ sea un espacio de Hilbert nos posibilita hacer cálculos en el espacio transformado:
$$
...
Observar que, en el Ejemplo 2.5.8, la transformación se hace entre dos espacios de dimensión finita, ya que allí $\calH=\RR^3$. No obstante, en general, es conveniente trabajar con $\calH$ de dimensión infinita. Más aún, de ahora en adelante consideraremos $\calH$ un espacio de funciones de la forma $h:\RR^p\to\RR$. Es decir, el mapeo será de la forma
$$
\xx\mapsto h.
$$
...
El cálculo de productos internos (2.5.12) en $\calH$ será fundamental para la generalización de SVM. Para un $\calH$ arbitrario, conocer explícitamente su producto interno $\langle\cdot,\cdot\rangle_{\calH}$ resulta ser una limitación práctica. Una manera de sortear esto es utilizar un *espacio de Hilbert con núcleo reproductor* (RKHS).


::: {.definicion}
**Definición 2.5.9.** (RKHS) Un espacio de Hilbert $\calH$ es un *espacio de Hilbert con núcleo reproductor* (RKHS) si admite un *núcleo reproductor* $k:\RR^p\times\RR^p\to\RR$ que verifica las siguientes propiedades:
...
Veamos el efecto de trabajar con RKHS para el cálculo de productos internos de la forma (2.5.12). Las propiedades de la Definición 2.5.9 implican:
...
<figure style="text-align: center;">
  ...
  <figcaption> **Figura 2.5.6**. Mapeo característico $\phi:\RR^p\to\calH$ asociado a un RKHS. </figcaption>
</figure>

<div style="margin-top:2em;"></div>

Este último resultado es crucial: permite afirmar que, dado un *mapeo característico* de la forma
$$
\xx\mapsto k(\xx,\cdot),
$$
...
::: {.definicion}
**Definición 2.5.10.** Una función $k:\RR^p\times\RR^p\to\RR$ se dice *definida positiva* si para todo $n\in\NN$, $\alpha_1,\ldots,\alpha_n\in\RR$ y $\xx_1,\ldots,\xx_n\in\RR^p$ resulta
$$
...
La expresión (2.5.13) se puede escribir matricialmente como $K\succeq 0$, donde $K\in\RR^{n\times n}$ es la *matriz de Gram* definida por
$$
K_{ij}=k(\xx_i,\xx_j).
$$
...
Para obtener el hiperplano óptimo de un conjunto de datos en un RKHS, los pasos a seguir son:

1. Se resuelve el problema dual (2.4.14) para obtener $\bflambda^\star$.
...
$$
donde $\mathcal{S}=\{i\mid \lambda_i^\star\neq 0\land\xi_i^\star=0\}$. Como antes, esto último resulta de despejar $\beta_0^\star$ de la condición de holgura complementaria para los puntos que verifican $\lambda_i^\star\neq 0$ y $\xi_i^\star=0$.
...
<table style="margin-left:auto; margin-right:auto; border-collapse: collapse; text-align: center; background-color: #f7faf7;">
  <tr style="border-bottom: 1px solid #cccccc;">
    ...
    <td style="padding: 16px 8px;">$\hat{f}(\xx)=\displaystyle\sum_{\substack{i=1\\\lambda_i^\star\neq 0}}^n\lambda_i^\star y_ik(\xx_i,\xx)+\beta_0^\star$</td>
  </tr>
  <tr>
...
::: {.callout-example}
<span class="badge bg-primary">Ejemplo 2.5.11</span> 
<span style="color: #0d6efd; font-family: Arial; font-weight: bold; font-size: 0.85em;">
